...
interface IUniqueSignerFactory {
     ...
     * @return magicValue Returns a legacy EIP-1271 magic value
     * (`bytes4(keccak256(isValidSignature(bytes,bytes))`) when the signature is valid. Reverting or
...
contract SafeSignerLaunchpad is IAccount, SafeStorage, SignatureValidatorConstants {
        ...
        returns (uint256 validationData)
    {
        ...
        if (missingAccountFunds > 0) {
            ...
            assembly ("memory-safe") {
                ...
                // enforces "strict" assembly blocks and "statements (elements of a block) are
                // disallowed if they return something onto the stack at the end."
    ...
    /**
     * @dev Validates that the user operation is correctly signed and returns an ERC-4337 packed
     * validation data
     * of `validAfter || validUntil || authorizer`:
     *  - `authorizer`: 20-byte address, 0 for valid signature or 1 to mark signature failure (this
     * module does not make use of signature aggregators).
     *  - `validUntil`: 6-byte timestamp value, or zero for "infinite". The user operation is valid
     * only up to this time.
        ...
        returns (uint256 validationData)
    {
        ...
        try IUniqueSignerFactory(signerFactory).isValidSignatureForSigner(
            operationHash, signature, signerData
        ) returns (bytes4 magicValue) {
            // The timestamps are validated by the entry point, therefore we will not check them
            // again
            validationData =
                _packValidationData(magicValue != EIP1271_MAGIC_VALUE, validUntil, validAfter);
        } catch {
