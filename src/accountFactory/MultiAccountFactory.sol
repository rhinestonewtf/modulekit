...
contract MultiAccountFactory is TestBase, Safe7579Factory, RefImplFactory, Kernel7579Factory {
    ...
    constructor() {
        ...
        } else if (keccak256(abi.encodePacked(_env)) == keccak256(abi.encodePacked(KERNEL7579))) {
    ...
    function _makeKernel(bytes calldata data, bytes32 salt) public returns (address) {
        ...
        returns (bytes memory init)
    {
        if (env == AccountType.SAFE7579) {
            ...
            init =
                abi.encodeCall(Kernel.initialize, (rootValidator, IHook(address(0)), initData, ""));
        } else {
            ERC7579BootstrapConfig[] memory _validators = generateConfig(validator, initData);
            ERC7579BootstrapConfig[] memory _executors = _emptyConfigs();
    ...
    //         // init = abi.encodeCall(Kernel.initialize, (rootValidator, hook, validatorData,
    // hookData));
    //     } else {
    //         init = abi.encode(
    //             address(bootstrapDefault),
