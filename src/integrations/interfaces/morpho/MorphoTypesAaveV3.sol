...
library BucketDLL {
    ...
    function insert(List storage _list, address _id, bool _head) internal returns (bool) {
        if (_head) {
...
library LogarithmicBuckets {
    ...
    /// @param _head Indicates whether to insert the new values at the head or at the tail of the
    /// buckets list.
    function update(
    ...
    /// @notice Returns the address in `_buckets` that is a candidate for matching the value
    /// `_value`.
    ...
    function _remove(Buckets storage _buckets, address _id, uint256 _bucket) private {
    ...
    function _insert(Buckets storage _buckets, address _id, uint256 _bucket, bool _head) private {
    ...
    /// @dev This is the same as rounding the input the nearest upper value of the form `2 ** n -
    /// 1`.
...
library DataTypesAaveV3 {
    ...
    struct UserConfigurationMap {
        /**
         * @dev Bitmap of the users collaterals and borrows. It is divided in pairs of bits, one
         * pair per asset.
         * The first bit indicates if an asset is used as collateral by the user, the second whether
         * an
    ...
    struct EModeCategory {
        ...
        // each eMode category may or may not have a custom oracle to override the individual assets
        // price oracles
        address priceSource;
        string label;
    }
...
interface IAavePoolAddressesProvider {
     ...
     */
    event ProxyCreated(
        bytes32 indexed id, address indexed proxyAddress, address indexed implementationAddress
    );
     ...
     * setting the new `PoolConfigurator` implementation when the function is called for the first
     * time.
...
library Types {
    ...
    struct LiquidityData {
        ...
        uint256 maxDebt; // The maximum debt value allowed before being liquidatable (in base
            // currency).
    ...
    struct MatchingEngineVars {
        ...
        bool borrow;
        function (address, address, uint256, uint256, bool) returns (uint256, uint256) updateDS; // This
            // function will be used to update the data-structure.
        bool demoting; // True for demote, False for promote.
        function(uint256, uint256, MarketSideIndexes256 memory, uint256)
            pure returns (uint256, uint256, uint256) step; // This function will be used to decide
            // whether to use the algorithm for promoting or for demoting.
    }
