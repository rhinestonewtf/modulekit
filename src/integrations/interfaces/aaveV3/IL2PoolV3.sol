...
interface IL2PoolV3 is IPoolV3 {
     ...
     * @dev the shortenedAmount is cast to 256 bits at decode time, if type(uint128).max the value
     * will be expanded to
     * type(uint256).max
    ...
    /**
     * @notice Calldata efficient wrapper of the repayWithPermit function, repaying on behalf of the
     * caller
     * @param args Arguments for the repayWithPermit function packed in one bytes32
     *    64 bits    8 bits        32 bits                   8 bits               128 bits       16
     * bits
     * | 0-padding | permitV | shortenedDeadline | shortenedInterestRateMode | shortenedAmount |
     * assetId |
     * @dev the shortenedAmount is cast to 256 bits at decode time, if type(uint128).max the value
     * will be expanded to
     * type(uint256).max
