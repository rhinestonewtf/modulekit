...
contract SessionKeyManagerHybrid is ISessionKeyManager, ERC7579ValidatorBase {
        ...
        * 0x0                  | 0x1               | 0x01 if sessionEnableTransaction, 0x00
        ...
        if (_isSessionEnableTransaction(moduleSignature)) {
            ...
            _verifySessionEnableDataSignature(
                sessionEnableData, sessionEnableSignature, userOp.getSender()
            );
            ...
            (bytes32 sessionDataDigest_, bytes calldata sessionKeySignature) =
                _parseSessionDataPreEnabledSignatureSingleCall(moduleSignature);

            SessionData storage sessionData =
                _validateSessionKeyPreEnabled(userOp.getSender(), sessionDataDigest_);

            rv = _packValidationData(
                //_packValidationData expects true if sig validation has failed, false otherwise
                !ISessionValidationModule(sessionData.sessionValidationModule).validateSessionUserOp(
                    userOp, userOpHash, sessionData.sessionKeyData, sessionKeySignature
                ),
    ...
    function _parseSessionEnableSignatureSingleCall(bytes calldata _moduleSignature)
        internal
        pure
        returns (
            uint256 sessionKeyIndex,
        ...
        )
    {
        ...
        *                      |                   |   sessionEnableSignature, sessionKeySignature)
        ...
        assembly ("memory-safe") {
            ...
            // baseOffset refers to the starting byte of this section, which starts with a list of
    ...
    function _parseSessionDataPreEnabledSignatureSingleCall(bytes calldata _moduleSignature)
        internal
        pure
        ...
        returns (uint48 validUntil, uint48 validAfter, address sessionKeyReturned)
    {
        ...
        _validateSessionKeySessionEnableTransaction(
            ...
            sessionValidationModule,
            sessionKeyData,
            _sessionEnableDataList[sessionKeyEnableDataIndex] // The signature has already been
                // verified
        );

        sessionKeyReturned = ISessionValidationModule(sessionValidationModule).validateSessionParams(
            _destination, _callValue, _operationCalldata, sessionKeyData, callSpecificData
        );
    }
        ...
        returns (uint48 validUntil, uint48 validAfter, address sessionKeyReturned)
    {
        ...
        SessionData storage sessionData =
            _validateSessionKeyPreEnabled(userOpSender, sessionDataDigest_);
        ...
        view
    {
        ...
        for (uint256 i = 0; i < length;) {
            _verifySessionEnableDataSignature(
                sessionEnableDataList[i], sessionEnableSignatureList[i], userOpSender
            );
            unchecked {
                ++i;
            }
    ...
    function _parseValidateUserOpBatchSignature(bytes calldata _moduleSignature)
        internal
        pure
        returns (
            bytes[] calldata sessionEnableDataList,
    ...
    function _parseSessionDataPreEnabledSignatureBatchCall(bytes calldata _moduleSignature)
        internal
        pure
    ...
    function _parseSessionEnableSignatureBatchCall(bytes calldata _moduleSignature)
        internal
        pure
        returns (
        ...
        )
    {
         ...
         * Offset (in bytes)    | Length (in bytes) | Contents
         * 0x0                  | 0x1               | Is Session Enable Transaction Flag
        * 0x1                  | 0x1               | Index of Session Enable Data in Session Enable
        Data List
        ...
        view
    {
        ...
        if (
            IERC1271(_smartAccount).isValidSignature(
                sessionEnableDataDigest, _sessionEnableSignature
            ) != EIP1271_SUCCESS
        ) {
        ...
        internal
    {
        (uint64 sessionChainId, bytes32 sessionDigest) =
            _parseSessionFromSessionEnableData(sessionEnableData, sessionKeyIndex);
        ...
        bytes32 computedDigest = _sessionDataDigestUnpacked(
            validUntil, validAfter, sessionValidationModule, sessionKeyData
        );
    ...
    {
        assembly ("memory-safe") {
            isSessionEnableTransaction := shr(248, calldataload(_moduleSignature.offset))
        }
    }

    function sessionDataDigest(SessionData calldata _data) public pure override returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                _data.validUntil,
                _data.validAfter,
    ...
    {
        return keccak256(
            abi.encodePacked(_validUntil, _validAfter, _sessionValidationModule, _sessionKeyData)
        );
    }

    function _isBatchExecuteCall(UserOperation calldata _userOp)
        internal
        pure
        returns (bool isBatchExecuteCall)
    {
        bytes4 selector = bytes4(_userOp.callData[0:4]);
        isBatchExecuteCall = selector == IERC7579Execution.executeBatch.selector;
    }
    ...
    function _parseSessionFromSessionEnableData(
        ...
        internal
        pure
        returns (uint64 sessionChainId, bytes32 sessionDigest)
    {
        ...
        assembly ("memory-safe") {
            ...
            sessionChainId := shr(192, calldataload(add(offset, mul(0x8, _sessionKeyIndex))))
            offset := add(offset, mul(0x8, enabledKeysCount))

            sessionDigest := calldataload(add(offset, mul(0x20, _sessionKeyIndex)))
        }
    ...
    function name() external pure virtual override returns (string memory) {
        return "SessionKeyManagerHybrid";
    }
