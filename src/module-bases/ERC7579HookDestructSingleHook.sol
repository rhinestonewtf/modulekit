...
abstract contract ERC7579HookDestructSingleHook is IERC7579Hook, ERC7579ModuleBase {
        ...
        returns (bytes memory hookData)
    {
        ...
        if (selector == IERC7579Account.execute.selector) {
            ...
            return onInstallModule(msg.sender, msgSender, moduleType, module, initData);
            ...
            return onUninstallModule(msg.sender, msgSender, moduleType, module, initData);
        } else {
            return onUnknownFunction(msg.sender, msgSender, msgValue, msgData);
        }
    }
        ...
        returns (bytes memory hookData)
    {
        ...
        if (calltype == CALLTYPE_SINGLE) {
            (address to, uint256 value, bytes calldata callData) =
                ExecutionLib.decodeSingle(encodedExecutions);
            return onExecuteFromExecutor(msg.sender, msgSender, to, value, callData);
        } else if (calltype == CALLTYPE_BATCH) {
            Execution[] calldata execs = ExecutionLib.decodeBatch(encodedExecutions);
            return onExecuteBatchFromExecutor(msg.sender, msgSender, execs);
        } else if (calltype == CALLTYPE_DELEGATECALL) {
            address to = address(bytes20(encodedExecutions[0:20]));
            bytes calldata callData = encodedExecutions[20:];
            return onExecuteDelegateCallFromExecutor(msg.sender, msgSender, to, callData);
        } else {
            revert InvalidCallType();
        }
    }
    ...
    function postCheck(bytes calldata hookData) external virtual override {
        onPostCheck(msg.sender, hookData);
    }
