...
library ValidatorSelectionLib {
    ...
    function getUserOpTarget(UserOperation calldata _op, uint offset) internal pure returns (address target) {
        offset + 4;
        target = address(bytes20(_op.callData[offset+12:offset+32]));
    }

    function getUserOpCallData(UserOperation calldata _op, uint offset)
        internal
        pure
