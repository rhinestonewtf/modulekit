...
contract GasliteSplitter {
    ...
    // event emitted when a payment is received (OpenZeppelin did this so I guess I have to do it
    // too)
    ...
    constructor(address[] memory _recipients, uint256[] memory _shares, bool _releaseRoyalty) {
        ...
        // hash of packed split slot, release royalty and totalShares are set outside of assembly
        // block
    ...
    function release() external {
        ...
        assembly {
            ...
            // canonical memAddresses array pointer, this returns the offset of the length of
            // memAddresses
            ...
            log1(
                sub(memAddresses, 0x40),
                add(0x40, mul(addrOffset, 0x02)),
                SPLIT_RELEASED_EVENT_SIGNATURE
            )
            stop()
        }
    }

    /// @notice Release all of given token (IERC20(_token).balanceOf(address(this))) to the
    /// recipients
    ...
    function release(address _token) external {
        ...
        assembly {
            ...
            // if `_token` has no code deployed to it, returndatacopy would revert since
            // returndata[(offset + length)] is greater than returndata.length
