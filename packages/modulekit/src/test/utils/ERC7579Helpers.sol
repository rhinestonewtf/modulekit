...
library ERC7579Helpers {
    ...
    ) internal returns (bytes memory erc7579Tx) {
        (address to, uint256 value, bytes memory callData) = fn(
            account,
            moduleType,
            module,
            initData
    ...
    function installModule(
        ...
        address module,
        bytes memory initData
    ) internal pure returns (address to, uint256 value, bytes memory callData) {
    ...
    function uninstallModule(
        ...
        address module,
        bytes memory initData
    ) internal view returns (address to, uint256 value, bytes memory callData) {
        if (moduleType == MODULE_TYPE_VALIDATOR) {
    ...
    function installValidator(
        ...
        callData = abi.encodeCall(
            ...
            (MODULE_TYPE_VALIDATOR, validator, initData)
        );
    }
    ...
    function uninstallValidator(
        ...
        (address[] memory array, ) = IAccountModulesPaginated(account)
            .getValidatorPaginated(address(0x1), 100);
    ...
    function installExecutor(
        ...
        callData = abi.encodeCall(
            IERC7579Account.installModule,
            (MODULE_TYPE_EXECUTOR, executor, initData)
        );
    }
    ...
    function installHook(
        ...
        callData = abi.encodeCall(
            IERC7579Account.installModule,
            (MODULE_TYPE_HOOK, hook, initData)
        );
    }
    ...
    function uninstallHook(
        ...
        callData = abi.encodeCall(
            IERC7579Account.uninstallModule,
            (MODULE_TYPE_HOOK, address(0), initData)
        );
    }
    ...
    function installFallback(
        ...
        callData = abi.encodeCall(
            IERC7579Account.installModule,
            (MODULE_TYPE_FALLBACK, fallbackHandler, initData)
        );
    }
    ...
    function uninstallFallback(
        ...
        callData = abi.encodeCall(
            IERC7579Account.uninstallModule,
            (MODULE_TYPE_FALLBACK, address(0), initData)
        );
    }
    ...
    function encode(
        ...
        uint256 value,
        bytes memory callData
    ) internal pure returns (bytes memory erc7579Tx) {
...
abstract contract BootstrapUtil {
    ...
    function _makeBootstrapConfig(
        address module,
        bytes memory data
    ) public pure returns (ERC7579BootstrapConfig memory config) {
    ...
    function makeBootstrapConfig(
        ...
        bytes[] memory datas
    ) public pure returns (ERC7579BootstrapConfig[] memory configs) {
...
library ArrayLib {
    function executions(
        Execution memory _1
    ) internal pure returns (Execution[] memory array) {
    ...
    ) internal returns (Execution[] memory result) {
